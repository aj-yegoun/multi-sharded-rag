...
def fast_oracle_relevant_document_coverage(
    *,
    ...
    """
    Fast Oracle Relevant Document Coverage@B.

    Instead of scanning every document inside selected shards,
    only check whether each relevant document belongs to a selected shard.
    This is much faster for large datasets such as FEVER and MS-MARCO.
    """
    if not relevant_docs:
        return 0.0

    selected_shard_set = set(selected_shards)
    hit_count = 0
    valid_relevant_count = 0

    for doc_id in relevant_docs:
        doc_id = str(doc_id)
        shard_id = doc_to_shard.get(doc_id)
        if shard_id is None:
            continue

        valid_relevant_count += 1
        if shard_id in selected_shard_set:
            hit_count += 1

    if valid_relevant_count == 0:
        return 0.0

    return hit_count / valid_relevant_count
    ...
    for top_b in top_b_values:
        ...
        for query_id, relevant_shards in relevant_shards_by_query.items():
            ...
            oracle_coverages.append(
                fast_oracle_relevant_document_coverage(
                    selected_shards=selected_shards,
